  ...
  return (
    <div style={{ background: 'white', borderRadius: 12, padding: 24 }}>
      ...
      <div style={{ width: '100%', overflow: 'hidden' }}>
        <Row gutter={[16, 16]} justify="start" style={{ minHeight: 200 }}>
          ...
          {/* 添加空占位Col，确保布局稳定 */}
          {TEMPLATES[activeKey as keyof typeof TEMPLATES].length < 3 && Array.from({ length: 3 - TEMPLATES[activeKey as keyof typeof TEMPLATES].length }).map((_, index) => (
            <Col xs={24} sm={12} md={8} key={`empty-${index}`}>
              <div style={{ height: 180, visibility: 'hidden', border: '1px solid transparent' }}></div>
            </Col>
          ))}
        </Row>
      </div>
